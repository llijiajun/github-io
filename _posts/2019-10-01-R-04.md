...
这里解读编译命令需要参考上一篇[文章]({{site.baseurl}}/2019-06-14/R-03)，这里不做赘述。唯一需要提取的是模板所在的位置，这里在 `--template` 后面的路径。找到路径后发现往上翻一下，有四个文件夹，css、fonts、images、templates，注意没有js的文件夹，但并不代表无法将js链接进模板中。最朴素的想法是，将js代码直接写到html里，虽然这并不是一种很好的编程风格。可以先从原理出发，把这些问题一一记下，之后我再一一解决。
...
![简单调头部的背景色]({{site.baseurl}}/images/R04-1.png)
